  ...
  def manyGran(self,*args,**kwargs):
    ...
    while 1:
      ...
      if surfflag == 0 and len(tris):
        surfflag = 1
        surface(tris)
        # 02/05/14 added file_egys by htm
      file, file_bb, file_walls, file_egys, file_rms = generateFilename(root,fileNos,n)
      ...
      particleEgys(file_egys,atoms,names,nvalues)
      rmsVel(file_rms,atoms,names,nvalues)
...
# -
# - 02/05/14
# - modified lines 192, 197, 202, 207 by htm to include a new file that will 
# - contain the energies
# ----------------------------------------------------------------------------
def generateFilename(root,fileNos,n):
  if fileNos[n] < 10:
    ...
    file_egys= root + "000" + str(fileNos[n]) + "_egys.txt"
    file_rms= root + "000" + str(fileNos[n]) + "_rms.txt"
  elif fileNos[n] < 100:
    file = root + "00" + str(fileNos[n]) + ".vtk"
    file_bb= root + "00" + str(fileNos[n]) + "_boundingBox.vtk"
    file_walls= root + "00" + str(fileNos[n]) + "_walls.vtk"
    file_egys= root + "00" + str(fileNos[n]) + "_egys.txt"
    file_rms= root + "00" + str(fileNos[n]) + "_rms.txt"
  elif fileNos[n] < 1000:
    file = root + "0" + str(fileNos[n]) + ".vtk"
    file_bb= root + "0" + str(fileNos[n]) + "_boundingBox.vtk"
    file_walls= root + "0" + str(fileNos[n]) + "_walls.vtk"
    file_egys= root + "0" + str(fileNos[n]) + "_egys.txt"
    file_rms= root + "0" + str(fileNos[n]) + "_rms.txt"
  else:
    file = root + str(fileNos[n]) + ".vtk"
    file_bb= root + str(fileNos[n]) + "_boundingBox.vtk"
    file_walls= root + str(fileNos[n]) + "_walls.vtk"
    file_egys= root + str(fileNos[n]) + "_egys.txt"
    file_rms= root + str(fileNos[n]) + "_rms.txt"

  return (file, file_bb, file_walls, file_egys, file_rms)
...
def particleEgys(file,atoms,names,n_values):
  f = open(file,"w")
  ...
  #atom[3],atom[4],atom[5]  = x,y,z  [atom[0]=id, atom[1]=type]
    #atom[3],atom[4],atom[5]  = x,y,z  [atom[0]=id, atom[1]=type]
...
def rmsVel(file,atoms,names,n_values):
  f = open(file,"w")
  ...
  # print head
  print >>f,"RMS fluctuations of velocity"
  print >>f,"Generated by lpp.py"
  print >>f,"POINTS %d float" % len(atoms)
  ...
  for atom in atoms:
    ...
    vy = atom[9]
    # m = atom[18]
    rms_vx = (vx**2).mean() - (vx.mean())**2
    rms_vy = (vy**2).mean() - (vy.mean())**2

  print >>f, rms_vx, rms_vy
  print >>f
  f.close()


# def inertialNumber_1(file,atoms,names,n_values):
  # f = open(file,"w")
  ...
  # # print head
  # print >>f,"Inertial number approximation "
  # print >>f,"Generated by lpp.py"
  # print >>f,"POINTS %d float" % len(atoms)
  # for atom in atoms:
  #   y = atom[3]
...
#   PythonCalculator1 = PythonCalculator(ArrayName='ys minus 4 times mean radius', Expression = 'inputs[0].Points[:,1]-4*mean(radius)', CopyArrays = 0)
#   Calculator1 = Calculator(AttributeMode = 'Point Data', ResultArrayName = 'ygt4mr', Function = 'ys minus 4 times mean radius>0' )
#   ClipDeposit = Clip( ClipType="Scalar", Scalars = ['POINTS', 'ygt4mr'] )
#   Delaunay2D1 = Delaunay2D( Alpha=0.01 )
#   FeatureEdges1 = FeatureEdges( )
#   Show()
#   # writer = CreateWriter("foo.csv", reader)
#   writer = CreateWriter(state.replace('vtk','csv'), FeatureEdges1)
